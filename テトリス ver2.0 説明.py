...
# PieceSizeは20pixで描写は0-19の値をとる
# y+(piecesize-1)は19を示す

#【関数】
#obj.GetEventObject = イベントを発生したオブジェクトを取得するメソッド

#break = for文やwhile文のブロックに使用
#break文以降の処理を抜けてfor文やwhile文の繰り返し処理を終了する

#return
#return文に処理が到達すると、その関数の実行は終わり、呼び出し元に制御が戻る
#return文で戻り値を指定していると、呼び出し元にそのデータを返す

# obj in obj  (例) 0 in obj[i]   ※obj[i]の中に0があるか?

#is = ojb同士を比較する演算子
#x=[1, 2]
#y=[1, 2]
# x == y => True    ※値同士の比較
# x is y => False   ※obj同士の比較
...
def OnKeyDown(event):
    ...
    if keycode == wx.WXK_LEFT or keycode == wx.WXK_RIGHT or\
        ...
        if keycode == wx.WXK_LEFT:
            ...
            for i in range(4):
                nextBlockCoords[i][0] = -curBlockCoords[i][1] #右辺の-curBlockCoords[i][x]で符号計算(マイナス化)もしている
